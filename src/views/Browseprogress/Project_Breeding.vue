    ...
    async enfetchData() {
      ...
      try {
        ...
        if (!this.enshowDetails) {
          ...
          response = await fetchLoopBreedingQuarterTableData(
            this.selectedProject,
            this.encurrentPage,
            this.enitemsPerPage
          );
          this.enquarterTableData = this.enorganizeTableData(response.data.results);
          responsechart = await fetchLoopBreedingAllQuarterChartData(this.selectedProject);
          this.enchartData = responsechart.data;
        } else {
