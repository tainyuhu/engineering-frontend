  ...
  <v-container>
    ...
    <div class="div-container" v-if="selectedLoopId && timeMode === 'week' && showDetails">
      <v-table>
        ...
        <tbody>
          <tr v-for="item in weekTableData" :key="item.voltage228k_name">
            <td class="font-weight-bold">{{ item.voltage228k_name }}</td>
    ...
    <div class="div-container" v-if="selectedLoopId && timeMode === 'quarter' && showDetails">
      <v-table>
        ...
        <tbody>
          <tr v-for="item in quarterTableData" :key="item.voltage228k_name">
            <td class="font-weight-bold">{{ item.voltage228k_name }}</td>
            <template v-for="dateRange in item.date_ranges">
              <td>{{ formatPercentage(dateRange.records[0].actual) }}</td>
              <td class="expected">{{ formatPercentage(dateRange.records[0].expected) }}</td>
    ...
    <div class="div-container" v-if="selectedLoopId && !showDetails && displayMode === 'table'">
      <v-table>
        ...
        <tbody>
          <tr v-for="item in TableData" :key="item.voltage228k_name">
            <td class="font-weight-bold">{{ item.voltage228k_name }}</td>
            <template v-for="dateRange in item.date_ranges">
              <td>{{ formatPercentage(dateRange.records[0].actual) }}</td>
              <td class="expected">{{ formatPercentage(dateRange.records[0].expected) }}</td>
    ...
    >
      ...
      >
        ...
        <div
          v-for="item in uniqueVoltage228KNames"
          :key="item.name"
          class="voltage228k-name-button"
        ...
        >
          <v-btn
            ...
            @click="selectVoltage228K(item.name)"
          >
            {{ item.name }}
          </v-btn>
        </div>
  ...
  methods: {
    organizeTableData(flatData) {
      ...
      flatData.forEach((item) => {
        const { voltage228k_name, date_range, actual, expected, year, quarter, week } = item;

        if (!tempMap.has(voltage228k_name)) {
          tempMap.set(voltage228k_name, { voltage228k_name, date_ranges: [] });
        }

        const currentVoltage228K = tempMap.get(voltage228k_name);

        let dateRangeObj = currentVoltage228K.date_ranges.find(
          (dr) => dr.date_range === date_range
        );
        if (!dateRangeObj) {
          dateRangeObj = { date_range, records: [], year, quarter, week };
          currentVoltage228K.date_ranges.push(dateRangeObj);
        }
    ...
    selectVoltage228K(voltage228kName) {
      console.log(`Selected Voltage228K: ${voltage228kName}`);
      // 選擇案場後的邏輯
    },
  },
};
