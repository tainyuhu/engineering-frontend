  ...
  <v-container>
    ...
    <div class="div-container" v-if="selectedLoopId && timeMode === 'week' && showDetails">
      <v-table>
        ...
        <tbody>
          <tr v-for="item in weekTableData" :key="item.voltage161k_name">
            <td class="font-weight-bold">{{ item.voltage161k_name }}</td>
    ...
    <div class="div-container" v-if="selectedLoopId && timeMode === 'quarter' && showDetails">
      <v-table>
        ...
        <tbody>
          <tr v-for="item in quarterTableData" :key="item.voltage161k_name">
            <td class="font-weight-bold">{{ item.voltage161k_name }}</td>
            <template v-for="dateRange in item.date_ranges">
              <td>{{ formatPercentage(dateRange.records[0].actual) }}</td>
              <td class="expected">{{ formatPercentage(dateRange.records[0].expected) }}</td>
    ...
    <div class="div-container" v-if="selectedLoopId && !showDetails && displayMode === 'table'">
      <v-table>
        ...
        <tbody>
          <tr v-for="item in TableData" :key="item.voltage161k_name">
            <td class="font-weight-bold">{{ item.voltage161k_name }}</td>
            <template v-for="dateRange in item.date_ranges">
              <td>{{ formatPercentage(dateRange.records[0].actual) }}</td>
              <td class="expected">{{ formatPercentage(dateRange.records[0].expected) }}</td>
    ...
    >
      ...
      >
        ...
        <div
          v-for="item in uniqueVoltage161KNames"
          :key="item.name"
          class="voltage161k-name-button"
        ...
        >
          <v-btn
            ...
            @click="selectVoltage161K(item.name)"
          >
            {{ item.name }}
          </v-btn>
        </div>
  ...
  methods: {
    organizeTableData(flatData) {
      ...
      flatData.forEach((item) => {
        const { voltage161k_name, date_range, actual, expected, year, quarter, week } = item;

        if (!tempMap.has(voltage161k_name)) {
          tempMap.set(voltage161k_name, { voltage161k_name, date_ranges: [] });
        }

        const currentVoltage161K = tempMap.get(voltage161k_name);

        let dateRangeObj = currentVoltage161K.date_ranges.find(
          (dr) => dr.date_range === date_range
        );
        if (!dateRangeObj) {
          dateRangeObj = { date_range, records: [], year, quarter, week };
          currentVoltage161K.date_ranges.push(dateRangeObj);
        }
    ...
    selectVoltage161K(voltage161kName) {
      console.log(`Selected Voltage161K: ${voltage161kName}`);
      // 選擇案場後的邏輯
    },
  },
};
