                    ...
                    <v-card :color="getProjectCardInfo(project.project_name).color" class="project-card" dark >
                        ...
                        <v-card-actions class="justify-end">
                            <template v-if="project.project_name === '案場'">
                                <v-btn :color="getProjectCardInfo(project.project_name).color" class="mt-auto font-weight-bold btn-custom" @click="navigateToProject(getProjectCardInfo(project.project_name).routeName, project.project_id)">瀏覽PV進度</v-btn>
                                <v-btn :color="getProjectCardInfo(project.project_name).color" class="mt-auto font-weight-bold btn-custom" @click="navigateToProject(getProjectCardInfo(project.project_name).routeName1, project.project_id)">瀏覽養殖進度</v-btn>
                            </template>
                            <template v-else>
                                <v-btn :color="getProjectCardInfo(project.project_name).color" class="mt-auto font-weight-bold btn-custom" @click="navigateToProject(getProjectCardInfo(project.project_name).routeName, project.project_id)">瀏覽進度</v-btn>
                            </template>
                        </v-card-actions>
                    </v-card>
                </v-col>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        // 根據項目名稱反應對應的名稱
        const projectInfo = {
            '案場': { title: '案場工程進度', color: '#00b894', icon: 'mdi-pier', routeName: 'Project_PV', routeName1: 'Project_Breeding' },
