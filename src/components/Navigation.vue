  ...
  <div>
    <v-sheet color="grey-lighten-4" class="px-6 py-8 v-sheet-custom">
      ...
      <v-row align="center" style="column-gap: 0.5rem">
        ...
        <h6 class="font-weight-black text-black text-h6">
    ...
    <v-list>
      ...
      <!--幫助與支持-->
      <v-list-group v-if="userHasGroup(['admin'])" no-action sub-group>
        <template v-slot:activator="{ props }">
          <v-list-item
...
const progress = computed(() => {
  const baseItems = [["瀏覽計畫進度", "mdi-eye", "Browse_Progress"]];
...
// 文件管理
const file = computed(() => {
  const baseItems = [["瀏覽文件進度", "mdi-file-eye-outline", "View_File_Progress"]];
...
const meeting = computed(() => {
  const baseItems = [["查詢紀錄", "mdi-magnify", "Search_Meeting_Record"]];
...
// 計劃管理
const plan = computed(() => {
  const baseItems = [["計畫列表", "mdi-format-list-bulleted", "Plan_List"]];
...
const master = computed(() => {
  const baseItems = [["總體計畫列表", "mdi-view-list", "Master_Plan_List"]];
